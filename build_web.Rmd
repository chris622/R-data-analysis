...
---
```{r}
setwd("D:/DOCTOR/Data_analysis/Rmarkdown/website")
...
## 创建仓库和R项目
+ github创建新仓库
```{r fig.cap='创建github仓库',echo=FALSE}
knitr::include_graphics('fig/create_repo.png')
```

+ 复制仓库URL
```{r fig.cap='复制仓库URL',echo=FALSE}
knitr::include_graphics('fig/clone_URL.png')
```

+ 创建和Github连接的R项目:`R studio-File-New project-Version control-Git`
```{r fig.cap='创建和Github连接的R项目',echo=FALSE}
knitr::include_graphics('fig/R_proj.png')
```
...
```{r fig.cap='模板主题',echo=FALSE}
knitr::include_graphics('fig/download.png')
```
...
```{r fig.cap='上传项目至github',echo=FALSE}
knitr::include_graphics('fig/commit.png')
knitr::include_graphics('fig/fetch.png')
```
+ 打开github可以看到项目内容已成功上传
```{r fig.cap='上传项目至github',echo=FALSE}
knitr::include_graphics('fig/update_repo.png')
```

# 使用netlify创建网站
+ 用github账号登录[netlify](https://app.netlify.com/signup/start)
+ `Add new site-Import an existing project-Github-deploy`
```{r fig.cap='deploy网站',echo=FALSE}
knitr::include_graphics('fig/add_site.png')
```
+ 更改`site settings`,`change site name`可以更改网站名
```{r fig.cap='更改site设置',echo=FALSE}
knitr::include_graphics('fig/site_settings.png')
```
+ 点击网站名即可跳转到建好的网站
```{r fig.cap='网站建好啦！',echo=FALSE}
knitr::include_graphics('fig/site.png')
```

+ 复制网站名，打开R studio项目文件，找到`config.yaml`，更改`baseURL`为复制好的网站名
```{r fig.cap='最后一步也完成啦！',echo=FALSE}
knitr::include_graphics('fig/change_URL.png')
```
